...
def make_label_from_disease(df_meta: pd.DataFrame) -> pd.Series:
    ...
    for _, row in df_meta.iterrows():
        ...
        if status in {"healthy", "obese"}:
            ...
            # fall back: Title 기반 (혹시 disease 항목이 비었을 때)
...
def align_samples(df_counts: pd.DataFrame, df_meta: pd.DataFrame) -> pd.DataFrame:
    ...
    if df_counts.shape[1] == df_meta.shape[0]:
        ...
        # 검증 로그
        print("\n[align_samples] 교집합이 없어서 '순서 매핑'으로 meta 인덱스를 counts 샘플ID로 변경.")
        print("[align_samples] 예시(앞 5개):")
        print(df_meta2.head(5)[["Title", "DiseaseStatus", "Label"]])
...
def ensembl_to_symbol(df_counts: pd.DataFrame) -> pd.DataFrame:
    ...
    # 같은 symbol이 여러 Ensembl로 존재할 수 있으니 합쳐줌
...
def main():
    ...
    if len(common_samples) == 0:
        raise ValueError("counts 샘플과 meta 샘플 교집합이 0이야. align_samples 로직을 확인해야 해.")

    # 4) Ensembl→Symbol 매핑 후 5 genes 선택 + log 변환
    ...
    if len(common_samples2) == 0:
        raise ValueError("symbol 변환 후 샘플 교집합이 0이야. meta 인덱스 정렬을 다시 확인해야 해.")
